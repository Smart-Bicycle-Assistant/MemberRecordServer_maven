...
public interface RidingLocationRepository extends JpaRepository<RidingLocation,String> {
    ...
    @Query(nativeQuery = true ,value="select l.id,l.longitude,l.latitude,l.speed from ( select *, ST_Distance_Sphere(point(m.longitude,m.latitude),point(:longitude,:latitude)) as distance from riding_location m ) l where l.distance < 5000 and (l.id != :memberId) and l.speed = :speed order by l.distance")
